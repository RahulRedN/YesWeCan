...
const Signup = () => {
  ...
  const onClickHandler = async (e) => {
    ...
    const conPass = confirmPassRef.current.value;
     ...
     if (email === "") {
       ...
       setError([
         true,
         {
           message: "Password should be atleast of length 8 and contain atleast 1 number",
           isPass: true,
         },
       ]);
       return;
    ...
    try {
      ...
      await addDoc(userDetailsRef, { email: email, name: name, mobile: mobile, photo: imageUrl, isLoggedIn: false, role: "online", uid: res.user.uid })
      await logout();
